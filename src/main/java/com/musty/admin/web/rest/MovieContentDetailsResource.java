...
import com.musty.admin.repository.MovieContentDetailsRepository;
...
public class MovieContentDetailsResource {
    ...
    @Timed
    public List<MovieContentDetails> getAllMovieContentDetails(@RequestParam(required = false) String filter) {
        if ("moviecontent-is-null".equals(filter)) {
            log.debug("REST request to get all MovieContentDetailss where movieContent is null");
            return StreamSupport
                .stream(movieContentDetailsRepository.findAll().spliterator(), false)
                .filter(movieContentDetails -> movieContentDetails.getMovieContent() == null)
